...
def cdist(a, b, metric='euclidean'):
    ...
    with tf.name_scope("cdist"):
        ...
        elif metric == 'min_euclidean':
            euclideanTop, indices = tf.math.top_k(tf.square(diffs), k=100, sorted=False, name=None)
            # euclideanTop2, indices2 = tf.math.top_k(tf.square(diffs), k=32, sorted=False, name=None)
            euclideanTopSum = tf.reduce_sum(euclideanTop, axis=-1)
            # euclideanTopSum2 = tf.reduce_sum(euclideanTop2, axis=-1)
            euclideanMinSum = tf.reduce_sum(tf.square(diffs), axis=-1) - 0.4*euclideanTopSum  + 1e-12
            return tf.sqrt(euclideanMinSum)
