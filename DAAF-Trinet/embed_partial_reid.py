...
def load_dataset_partial_reid(csv_file, image_root, fail_on_missing=False):
    img_path = []
    # file_id = []
    person_id = []
    camera_id = []

    # file_list = os.listdir(os.path.join(image_root, 'partial_body_images'))
    ...
    for file_name in file_list:
        ...
        # file_id.append(file_name[0:3] + file_name[6])
        person_id.append(file_name[0:3])
        camera_id.append(file_name[6])

    pids = np.array(person_id)
    fids = np.array(img_path)
    cids = np.array(camera_id)
...
def main():
    ...
    _, data_fids = load_dataset_partial_reid(args.dataset, args.image_root)
